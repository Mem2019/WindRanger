...
namespace {
	...
	class WindRangerInst : public ModulePass {

	public:
		static char ID;
		WindRangerInst() : ModulePass(ID) { }

		bool runOnModule(Module &M) override { inst_wr(M); return true; }

	};

	char WindRangerInst::ID = 0;
}

static void registerWRPass(
	const PassManagerBuilder &, legacy::PassManagerBase &PM) {

	PM.add(new WindRangerInst());

}

static RegisterStandardPasses RegisterAFLPassLTO(
    PassManagerBuilder::EP_FullLinkTimeOptimizationLast, registerWRPass);

static RegisterStandardPasses RegisterAFLPass0(
    PassManagerBuilder::EP_EnabledOnOptLevel0, registerWRPass);